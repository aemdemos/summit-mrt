...
function ensureTablistClickDelegation(block, tablist) {
  ...
  tablist.addEventListener('click', (e) => {
    const button = e.target.closest('button.tabs-brands-tab');
    ...
    tablist.querySelectorAll('button.tabs-brands-tab').forEach((btn) => {
...
function isTabRowCandidate(row, tablist) {
  ...
  if (row.matches('.tabs-brands-panel[role="tabpanel"]')) {
    return true;
  }
...
export function resyncTabsBlock(block) {
  const tablist = block.querySelector(':scope > .tabs-brands-list');
  if (!tablist) {
    return;
  }
  ...
  const openResource = block.querySelector('.tabs-brands-panel[aria-hidden="false"]')?.getAttribute('data-aue-resource');
  ...
  rows.forEach((row, i) => {
    ...
    if (!row.matches('.tabs-brands-panel[role="tabpanel"]')) {
      ...
      row.className = 'tabs-brands-panel';
      row.id = id;
  ...
  tablist.querySelectorAll(':scope > button.tabs-brands-tab').forEach((btn, i) => {
