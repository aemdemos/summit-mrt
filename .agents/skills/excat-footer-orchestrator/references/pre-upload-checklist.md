...
- **Avoid plan vs code drift:** Gate behavior is defined in repo hooks (`.agents/hooks/footer-validation-gates/`, `.agents/hooks/footer-validation-gate.js`). Changing rules there requires updating `references/footer-validation-gates-summary.md` (and SKILL or troubleshooting when user-visible). The skill’s `scripts/pre-completion-check.js` is canonical; workspaces must recopy from `skills/excat-footer-orchestrator/scripts/` when the skill changes.
