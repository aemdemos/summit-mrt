...
9. **Deep megamenu mapping (REQUIRED when any dropdown exists):** After writing phase-3, perform a **per-item deep analysis** of every dropdown panel (megamenu = any panel that drops down on hover/click, small or large):
   ...
   - **Content destination: nav.plain.html (CRITICAL):** Follow the **`nav.plain.html` contract** under **Implementation (EDS)** — one portable fragment for local + DA/EDS. All text, link labels, category names, sub-menu items, promotional text, link URLs, **icon cards**, lists, and images discovered during deep megamenu mapping belong in `content/nav.plain.html` as plain-friendly markup. **Search bars and form controls** (`<form>`, `<input>`, `<button>`, `<label>`) are built in `header.js`, not embedded in the fragment. The JS code fetches the plain fragment and reads DOM content; it does not invent *copy*. **If you notice "source had X that wasn't included":** (1) Write `missing-content-register.json` with the omission (`location`, `description`, `resolved: false`). (2) Add X to nav.plain.html (or implement controls in `header.js` per contract). (3) Extract the exact styles from the source site so we match them precisely. (4) Set `resolved: true` in the register. (5) Re-run validate-nav-content.js. The hook blocks until all resolved. Plan structure so `header.js` can traverse and render faithfully.
...
3. **Implement desktop only:**
   ...
   - **Content-first architecture (CRITICAL — do NOT hardcode text/content in JS):** ALL megamenu text content, link labels, category names, sub-menu items, promotional text, and link URLs MUST live in `content/nav.plain.html` as **plain-friendly** markup — NOT duplicated as hardcoded strings in `header.js`. The **source site is authoritative**. **Form controls and search inputs** follow the same **`nav.plain.html` contract** as footer: put structure and copy in the fragment; build `<form>` / `<input>` / `<button>` / `<label>` in `header.js`. If you notice "source had X that wasn't included", STOP and add X to the fragment (or add the control in JS). `header.js` should: (1) fetch the plain fragment (dual-fetch under **Implementation (EDS)**) and parse its DOM, (2) build panels/accordions/grids and interactive controls from that content, (3) add event handlers. If the source megamenu has "CategoryA → SubCat → Product" with thumbnail + specs, text/links/image refs go into nav.plain.html; interactive pieces are created in JS. NEVER create site-specific function names (e.g. `buildCategoryAMegamenu`) — all functions must be generic and reusable across any site.
...
- **`nav.plain.html` contract:** Same idea as footer — **one fragment** for local preview and DA/EDS. Put portable semantic content in `content/nav.plain.html` (headings, lists, links, images, section shells). Put **form controls**, **search inputs**, layout that depends on `class` / `data-*` / `style` / `id` on wrappers, and deep layout `<div>` shells in `header.js` + `header.css`. Document Authoring is the strictest consumer; do not rely on “GitHub-only” HTML inside plain.html.
...
- **Content-first:** ALL text/links/labels that belong in the fragment live in `content/nav.plain.html`. JS reads, augments with controls, and renders — never generates *copy* that should live in plain.html.

- **Dual-fetch for `header.js` (this order is correct):** (1) `fetch('/content/nav.plain.html')` — localhost / aem up; (2) if not OK, `fetch` with `` `${navPath}.plain.html` `` — DA/EDS production. Set `navPath` from block metadata (path to the nav doc **without** the `.plain.html` suffix).
...
- Hardcode megamenu text/links/labels in header.js — that copy belongs in `content/nav.plain.html` per the **`nav.plain.html` contract**; JS reads and presents it.
- **CRITICAL — DA/EDS flat structure (`content/nav.plain.html`):** The fragment must stay **flat and semantic**: only **top-level section `<div>`s** (no nested layout `<div>`s inside a section); **no** `<form>`, `<input>`, `<button>`, `<label>`, `<select>`, or `<textarea>`; **no** `class=`, `data-*`, `style=`, or `id=` on markup in the fragment — put layout, controls, and selectors in **`header.js`** + **`header.css`** after fetch. This is enforced by the PostToolUse **`FLAT_STRUCTURE`** gate and by **`validate-nav-content.js`**; both must pass (see **`nav.plain.html` contract** and `references/element-handling-guide.md`).
