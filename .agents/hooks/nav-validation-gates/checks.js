...
/**
 * PostToolUse: flat semantic structure for content/nav.plain.html.
 * Returns null (pass) if not nav content path or unreadable; otherwise error string or null if clean.
 */
export function checkNavContentFlatStructure(filePath) {
  const base = path.basename(filePath || '');
  if (base !== 'nav.plain.html') return null;
  let html = '';
  try {
  ...
  const { failures } = checkFlatSemanticStructure(html, { jsBlockName: 'header.js' });
  if (failures.length === 0) return null;
  return `FLAT_STRUCTURE — content/nav.plain.html must be DA/EDS-safe:\n\n${failures.map((f) => `  • ${f}`).join('\n')}`;
}
