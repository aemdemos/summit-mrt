 ...
 * When editing nav / header / validation files, the validation dir is created if missing so `debug.log` is always written.
...
function handlePostToolUse(hookInput) {
  ...
  if (!workspaceDebugLog && (isNavContentFile(filePath) || isNavValidationFile(filePath) || isHeaderFile(filePath))) {
  ...
  if (result.decision === 'block') {
    ...
    if (reason.includes('FLAT_STRUCTURE')) {
      log('BLOCK', '[PostToolUse] BLOCK — FLAT_STRUCTURE — write to nav.plain.html rejected (full detail in Gate FLAT_STRUCTURE FAILED log entry above)');
    }
...
function handleStop(hookInput) {
  ...
  if (navFilePath) {
    const flatErr = checkNavContentFlatStructure(navFilePath);
    if (flatErr) {
      allErrors.push(flatErr);
      log('INFO', '[FLAT_STRUCTURE] Stop check: nav.plain.html fails flat semantic structure', { detail: flatErr.slice(0, 800) });
    }
